...
class VALUCvtSpec extends AnyFlatSpec {
  ...
  private def runS32ToF32(dut: VALU): Unit = {
    zeroInputs(dut)
    pokeCtrl(dut, VecOp.vcvt_s32_f32)  // s32→f32
  ...
  private def runF32ToS32(dut: VALU): Unit = {
    zeroInputs(dut)
    pokeCtrl(dut, VecOp.vcvt_f32_s32)  // f32→s32
  ...
  private def runF32ToS8(dut: VALU): Unit = {
    zeroInputs(dut)
    pokeCtrl(dut, VecOp.vcvt_f32_s8, sat = true)  // f32→s8
  ...
  private def runBf16RoundTrip(dut: VALU): Unit = {
    zeroInputs(dut)
  ...
  private def runBf8E4M3(dut: VALU): Unit = {
    zeroInputs(dut)
  ...
  "VALU vcvt" should "pass all conversion sub-cases" in {
    simulate(new VALU(K, N)) { dut =>
      withClue("s32->f32: ")             { runS32ToF32(dut)      }
      withClue("f32->s32 (RTZ): ")       { runF32ToS32(dut)      }
      withClue("f32->s8 saturating: ")   { runF32ToS8(dut)       }
      withClue("f32<->bf16 round-trip: ") { runBf16RoundTrip(dut) }
      withClue("f32<->bf8 E4M3: ")        { runBf8E4M3(dut)       }
    }
  }
}
