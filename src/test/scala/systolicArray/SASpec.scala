    ...
    "SA" should "do a normal matrix multiplication" in {
        test(new SystolicArray(4, 8)) { dut =>
            ...
            for (i_tick <- 0 to 3 * _n - 2) {
                ...
                // this.printMatrixChisel(dut.io.out, _n)
            ...
            for (i <- 0 until _n) {
                for (j <- 0 until _n) {
                    dut.io.out(i * _n + j).expect(_expected(i * _n + j))
                }
            }
        }
    }

    // TODO: clarify anti-bubble scheduling
    "SA" should "avoid schedule dispatch bubbles" in {
        test(new SystolicArray(4, 8)) { dut =>
            val _n = 4
        }
    }
