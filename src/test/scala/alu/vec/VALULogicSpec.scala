...
class VALULogicSpec extends AnyFlatSpec {
  ...
  private def runVand(dut: VALU): Unit = {
  ...
  private def runVor(dut: VALU): Unit = {
  ...
  private def runVxor(dut: VALU): Unit = {
  ...
  private def runVnot(dut: VALU): Unit = {
  ...
  private def runVsll(dut: VALU): Unit = {
  ...
  private def runVsra(dut: VALU): Unit = {
  ...
  "VALU logic" should "pass all bitwise / shift sub-cases" in {
    simulate(new VALU(K, N)) { dut =>
      withClue("vand: ") { runVand(dut) }
      withClue("vor: ")  { runVor(dut)  }
      withClue("vxor: ") { runVxor(dut) }
      withClue("vnot: ") { runVnot(dut) }
      withClue("vsll: ") { runVsll(dut) }
      withClue("vsra: ") { runVsra(dut) }
    }
  }
}
